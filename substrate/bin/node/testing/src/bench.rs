...
use argochain_runtime::{
	constants::currency::ARGO, AccountId, BalancesCall, CheckedExtrinsic, MinimumPeriod,
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((
					sender,
					signed_extra(0, argochain_runtime::ExistentialDeposit::get() + 1),
				)),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive =>
						RuntimeCall::Balances(BalancesCall::transfer_keep_alive {
							dest: sp_runtime::MultiAddress::Id(receiver),
							value: argochain_runtime::ExistentialDeposit::get() + 1,
						}),
					BlockType::RandomTransfersReaping => {
						RuntimeCall::Balances(BalancesCall::transfer_allow_death {
							...
							value: 100 * ARGO -
								(argochain_runtime::ExistentialDeposit::get() - 1),
						})
					},
					BlockType::Noop =>
