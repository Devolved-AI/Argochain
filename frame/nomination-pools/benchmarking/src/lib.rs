...
fn vote_to_balance<T: pallet_nomination_pools::Config>(
    ...
    vote.try_into()
        .map_err(|_| "could not convert u64 to Balance")
}
...
impl<T: Config> ListScenario<T> {
    ...
    pub(crate) fn new(
        ...
        let dest_weight: BalanceOf<T> = dest_weight_as_vote
            .try_into()
            .map_err(|_| "could not convert u64 to Balance")?;
        ...
        assert_eq!(
            vote_to_balance::<T>(weight_of(&pool_origin1)).unwrap(),
            origin_weight
        );
        assert_eq!(
            vote_to_balance::<T>(weight_of(&pool_origin2)).unwrap(),
            origin_weight
        );
        assert_eq!(
            vote_to_balance::<T>(weight_of(&pool_dest1)).unwrap(),
            dest_weight
        );
    ...
    fn add_joiner(mut self, amount: BalanceOf<T>) -> Self {
        ...
        let weight_of = pallet_staking::Pallet::<T>::weight_of_fn();
        assert_eq!(
            vote_to_balance::<T>(weight_of(&self.origin1)).unwrap(),
            original_bonded
        );
