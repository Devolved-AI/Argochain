...
pub const DEFAULT_ROLES: PoolRoles<AccountId> = PoolRoles {
    depositor: 10,
    root: Some(900),
    nominator: Some(901),
    bouncer: Some(902),
...
fn deposit_rewards(r: u128) {
    let b = Balances::free_balance(&default_reward_account())
        .checked_add(r)
        .unwrap();
...
mod unbond_pool {
    ...
    fn points_to_issue_works() {
        ExtBuilder::default().build_and_execute(|| {
            // 1 points : 1 balance ratio
            let unbond_pool = UnbondPool::<Runtime> {
                points: 100,
                balance: 100,
            };
    ...
    fn claim_payout_other_works() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_eq!(
                    claimable_reward, 3,
                    "test is correct if rewards are divisible by 3"
                );
...
mod unbond {
    ...
    fn member_unbond_open() {
        ...
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_eq!(
                    PoolMembers::<Runtime>::get(20).unwrap().unbonding_points(),
                    5
                );
                ...
                // Make permissionless
                assert_eq!(
                    ClaimPermissions::<Runtime>::get(10),
    ...
    fn depositor_unbond_open() {
        ...
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_eq!(
                    PoolMembers::<Runtime>::get(10).unwrap().unbonding_points(),
                    5
                );
    ...
    fn unbond_of_1_works() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_eq!(
                StakingMock::active_stake(&default_bonded_account()).unwrap(),
                0
            );
        });
    }
    ...
    fn unbond_kick_works() {
        ...
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_eq!(
                    *UnbondingBalanceMap::get()
                        .get(&default_bonded_account())
                        .unwrap(),
                    100 + 200
                );
            });
...
mod pool_withdraw_unbonded {
    ...
    fn pool_withdraw_unbonded_works() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            // When
            assert_ok!(Pools::pool_withdraw_unbonded(
                RuntimeOrigin::signed(10),
                1,
                0
            ));
    ...
    fn withdraw_unbonded_works_against_slashed_no_era_sub_pool() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                // `no_era`
                let sub_pools = SubPoolsStorage::<Runtime>::get(1)
                    .unwrap()
    ...
    fn partial_withdraw_unbonded_non_depositor() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_eq!(
                    PoolMembers::<Runtime>::get(11).unwrap().unbonding_points(),
                    7
                );
                assert_eq!(
                    pool_events_since_last_call(),
...
fn set_claimable_actor_works() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        // Make permissionless
        assert_eq!(
            ClaimPermissions::<Runtime>::get(11),
            ClaimPermission::Permissioned
        );
        assert_noop!(
        ...
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    Event::StateChanged {
                        pool_id: 1,
                        new_state: PoolState::Blocked
                    }
                ]
            );

            assert_eq!(
                BondedPool::<Runtime>::get(1).unwrap().state,
                PoolState::Blocked
            );
...
mod set_configs {
    ...
    fn set_configs_works() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_eq!(
                GlobalMaxCommission::<Runtime>::get(),
...
mod bond_extra {
    ...
    fn bond_extra_from_rewards_creator() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_eq!(Balances::free_balance(10), 35);
            assert_eq!(
                PoolMembers::<Runtime>::get(10).unwrap().points,
                10 + claimable_reward
            );
            assert_eq!(
                BondedPools::<Runtime>::get(1).unwrap().points,
                10 + claimable_reward
            );
...
mod commission {
    ...
    fn set_commission_works() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_eq!(
                pool_events_since_last_call(),
                vec![Event::PoolCommissionClaimed {
                    pool_id: 1,
                    commission: 20
                }]
            );
