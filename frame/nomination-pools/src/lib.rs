...
#[cfg_attr(
    feature = "std",
    derive(frame_support::PartialEqNoBound, DefaultNoBound)
)]
...
impl<T: Config> Commission<T> {
    ...
    fn throttling(&self, to: &Perbill) -> bool {
        if let Some(t) = self.change_rate.as_ref() {
            let commission_as_percent = self
                .current
                .as_ref()
                .map(|(x, _)| *x)
    ...
    fn try_update_change_rate(
        ...
        change_rate: CommissionChangeRate<BlockNumberFor<T>>,
            ...
            !&self.less_restrictive(&change_rate),
            Error::<T>::CommissionChangeRateNotAllowed
        );
...
impl<T: Config> BondedPool<T> {
    ...
    pub fn get(id: PoolId) -> Option<Self> {
        BondedPools::<T>::try_get(id)
            .ok()
            .map(|inner| Self { id, inner })
    }
    ...
    fn can_nominate(&self, who: &T::AccountId) -> bool {
        self.is_root(who)
            || self
                ...
                .nominator
                .as_ref()
                .map_or(false, |nominator| nominator == who)
    }
    ...
    fn ok_to_unbond_with(
        ...
        let balance_after_unbond = {
            let new_depositor_points = target_member
                .active_points()
                .saturating_sub(unbonding_points);
...
impl<T: Config> RewardPool<T> {
    ...
    fn update_records(
        ...
        self.total_commission_pending = self
            .total_commission_pending
            .saturating_add(new_pending_commission);
...
impl<T: Config> SubPools<T> {
    ...
    fn sum_unbonding_balance(&self) -> BalanceOf<T> {
        self.no_era.balance.saturating_add(
            self.with_era
                .values()
                .fold(BalanceOf::<T>::zero(), |acc, pool| {
    ...
    impl<T: Config> Pallet<T> {
            ...
            let balance_to_unbond = withdrawn_points
                ...
                .fold(
                    ...
                    |accumulator, (era, unlocked_points)| {
                        sum_unlocked_points = sum_unlocked_points.saturating_add(*unlocked_points);
                        log::info!(
                            "Processing era: {:?}, unlocked_points: {:?}",
                            era,
                            unlocked_points
                        );
                        ...
                        if let Some(era_pool) = sub_pools.with_era.get_mut(era) {
                            ...
                            log::info!(
                                "Balance to unbond from no era pool: {:?}",
                                balance_to_unbond
                            );
                            accumulator.saturating_add(balance_to_unbond)
                        }
        ...
        pub fn create_with_pool_id(
            ...
            let depositor = ensure_signed(origin)?;
                ...
                !BondedPools::<T>::contains_key(pool_id),
                Error::<T>::PoolIdInUse
            );
        ...
        pub fn set_metadata(
            ...
            let who = ensure_signed(origin)?;
            let metadata: BoundedVec<_, _> = metadata
                .try_into()
                .map_err(|_| Error::<T>::MetadataExceedsMaxLen)?;
            ensure!(
            ...
            let mut bonded_pool = match ensure_root(origin.clone()) {
                ...
                Err(frame_support::error::BadOrigin) => {
                    ...
                    let bonded_pool =
                        ...
                        bonded_pool.can_update_roles(&who),
                        Error::<T>::DoesNotHavePermission
                    );
                    bonded_pool
                }
        ...
        pub fn set_commission(
            ...
            let mut bonded_pool = BondedPool::<T>::get(pool_id).ok_or(Error::<T>::PoolNotFound)?;
                ...
                bonded_pool.can_manage_commission(&who),
                Error::<T>::DoesNotHavePermission
            );
        ...
        pub fn set_commission_max(
            ...
            bonded_pool
                .commission
                .try_update_max(pool_id, max_commission)?;
            bonded_pool.put();
...
impl<T: Config> Pallet<T> {
    ...
    pub fn dissolve_pool(bonded_pool: BondedPool<T>) {
        ...
        log::info!(
            "Removing ReversePoolIdLookup entry for bonded_account: {:?}",
            bonded_account
        );
        ReversePoolIdLookup::<T>::remove(&bonded_account);
        ...
        log::info!(
            "Removing RewardPools entry for pool ID: {:?}",
            bonded_pool.id
        );
        RewardPools::<T>::remove(bonded_pool.id);
        ...
        log::info!(
            "Removing SubPoolsStorage entry for pool ID: {:?}",
            bonded_pool.id
        );
        SubPoolsStorage::<T>::remove(bonded_pool.id);
        ...
        log::info!(
            "Setting reward_account balance to zero: {:?}",
            reward_account
        );
        T::Currency::make_free_balance_be(&reward_account, Zero::zero());

        log::info!(
            "Setting bonded_account balance to zero: {:?}",
            bonded_account
        );
        T::Currency::make_free_balance_be(&bonded_pool.bonded_account(), Zero::zero());
        ...
        // Deposit event
        log::info!(
            "Depositing Destroyed event for pool ID: {:?}",
            bonded_pool.id
        );
        ...
        log::info!(
            "Removing bonded pool entry for pool ID: {:?}",
            bonded_pool.id
        );
        bonded_pool.remove();
    }
    ...
    fn do_create(
        ...
        let bouncer = T::Lookup::lookup(bouncer)?;
            ...
            amount >= Pallet::<T>::depositor_min_bond(),
            Error::<T>::MinimumBondNotMet
        );
        ensure!(
    ...
    fn do_bond_extra(
        ...
        let (points_issued, bonded) = match extra {
            ...
            BondExtra::Rewards => (
                bonded_pool.try_bond_funds(&who, claimed, BondType::Later)?,
                claimed,
            ),
        };
    ...
    pub fn do_try_state(level: u8) -> Result<(), TryRuntimeError> {
        ...
        PoolMembers::<T>::iter().try_for_each(|(_, d)| -> Result<(), TryRuntimeError> {
            let bonded_pool = BondedPools::<T>::get(d.pool_id).unwrap();
                ...
                "No member should have zero points"
            );
    ...
    pub fn fully_unbond(
        ...
        let points = PoolMembers::<T>::get(&member)
