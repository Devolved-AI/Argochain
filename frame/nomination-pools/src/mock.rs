...
pub fn pool_events_since_last_call() -> Vec<super::Event<Runtime>> {
    let events = System::events()
        ...
        .map(|r| r.event)
        .filter_map(|e| {
            if let RuntimeEvent::Pools(inner) = e {
                Some(inner)
            } else {
                None
            }
...
pub fn balances_events_since_last_call() -> Vec<pallet_balances::Event<Runtime>> {
    let events = System::events()
        ...
        .map(|r| r.event)
        .filter_map(|e| {
            if let RuntimeEvent::Balances(inner) = e {
                Some(inner)
            } else {
                None
            }
...
mod test {
    ...
    fn u256_to_balance_convert_works() {
        ...
        assert_eq!(
            U256ToBalance::convert(Balance::max_value().into()),
            Balance::max_value()
        )
    }
    ...
    fn balance_to_u256_convert_works() {
        ...
        assert_eq!(
            BalanceToU256::convert(Balance::max_value()),
